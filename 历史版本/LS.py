...
8. generate_report: 生成选股报告，展示排名前50的股票
...
class Constants:
    ...
    BASE_DIR = "rxxg/quant_data"    # 数据存放目录
...
class Datasr(metaclass=Singleton):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.logger = get_logger('Datasr', log_file)
    ...
    def fetch_data(self, symbol: str, days: Optional[int] = None, min_data_ratio: Optional[float] = None) -> pd.DataFrame:
        ...
        try:
            ...
            # 需要更新数据
            return self._update_stock_data(symbol, days, cache_path, cached_df, min_data_ratio)
            
        except Exception as e:
            error_msg = f"获取股票 {symbol} 数据处理失败: {str(e)}"
    ...
    def _load_from_cache(self, symbol: str, cache_path: str) -> Tuple[pd.DataFrame, bool]:
        ...
        self.logger.info(f"从本地缓存加载 {symbol} 数据")
        try:
            # 尝试从缓存文件读取数据
            ...
            # 判断是否需要更新
            business_days_diff = len(pd.bdate_range(last_date, today)) - 1
            
            if business_days_diff <= 1 :  # 只有1天差距
                self.logger.info(f"缓存数据已是最新 ({last_date})，无需更新")
                return cached_df, False
            self.logger.info(f"缓存数据需要增量更新，最后日期: {last_date}")
    ...
    def _update_stock_data(self, symbol: str, days: int, cache_path: str, 
                          cached_df: pd.DataFrame, min_data_ratio: float) -> pd.DataFrame:
        """更新股票数据并处理增量更新逻辑"""
        # 获取原始数据
        raw_df = self._fetch_from_source(symbol, days)
        if raw_df.empty:
            self.logger.warning(f"股票 {symbol} 原始数据为空，跳过处理")
            # 如果有缓存则返回缓存，否则返回空DataFrame
        ...
        if not cached_df.empty:
            result_df = self._merge_with_cache(cached_df, new_df, days)
        else:
            # 没有缓存，直接使用新数据
            result_df = new_df
            if len(result_df) > days:
                result_df = result_df.sort_index().tail(days)
    ...
    def _merge_with_cache(self, cached_df: pd.DataFrame, new_df: pd.DataFrame, days: int) -> pd.DataFrame:
        """合并缓存数据和新数据"""
        # 找出新数据中有而缓存中没有的记录
        last_date = cached_df.index.max()
        ...
        if new_data.empty:
            self.logger.info("没有新数据可更新")
            return cached_df
        
        self.logger.info(f"获取到 {len(new_data)} 条新数据，进行增量更新")
        # 合并新旧数据并排序
        result_df = pd.concat([cached_df, new_data]).sort_index()
        
        # 如果数据量超过要求天数，只保留最近的数据
        if len(result_df) > days:
    ...
    def _fetch_from_source(self, symbol: str, days: int) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                self.logger.warning("主接口数据为空，尝试备用方法")
                df = self._fetch_with_retry(ak.stock_zh_a_hist, symbol=symbol, period="daily", adjust="")
...
class TechnicalIndicators:
    ...
    def _calculate_limit_up_probability(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            # 各因子权重
            weights = {
                'pattern': 0.25,  # K线形态 - 25%
                'tech': 0.25,     # 技术指标 - 25% 
                'volume': 0.20,   # 成交量特征 - 20%
                'price': 0.10,    # 价格趋势 - 10%
                'market': 0.10,   # 市场因素 - 10% (来自其他计算)
                'reversal': 0.10  # 反转信号 - 10% (来自其他计算)
            }
...
def calculate_stock_score(df: pd.DataFrame, params: TradingParams) -> pd.DataFrame:
    ...
    try:
        ...
        if '收盘强度' not in df.columns:
            df['收盘强度'] = (df['Close'] - df['Low']) / (df['High'] - df['Low'] + Constants.EPSILON)
        
        if '量比' not in df.columns and 'Volume' in df.columns:
            vol_ma5 = df['Volume'].rolling(5).mean().replace(0, Constants.EPSILON)
        ...
        # =====================================================================
        # 1. 价格趋势评分 (最高20分) - 评估价格走势和K线形态
        # =====================================================================
        
        # 1.1 收盘强度评分 (0-8分) - 收盘位置靠近最高价更有利于次日上涨
        # 收盘强度越高，说明买盘越强，上涨概率越大
        ...
        if all(col in df.columns for col in ['MA5', 'MA10', 'MA20']):
            ...
            # 均线多头排列得分（最高6分）- 优化：增加均线趋势角度因素
            ma_score = price_above_ma5.astype(int) * 2 + \
        ...
        # 1.3 K线形态评分 (0-6分) - 使用TA-Lib识别的K线形态
        # K线形态是短线交易的重要参考指标
        pattern_score = np.zeros(len(df))
        ...
        if 'Pattern_Score' in df.columns:
            ...
            if 'Pattern_MorningStar' in df.columns:
                pattern_score += df['Pattern_MorningStar'].astype(int) * 5  # 提高晨星形态权重
                
            # 三白兵 - 强势上涨信号
            if 'Pattern_3WhiteSoldiers' in df.columns:
                pattern_score += df['Pattern_3WhiteSoldiers'].astype(int) * 5  # 提高三白兵权重
                
            # 吞没形态 - 重要的反转信号
            if 'Pattern_Engulfing' in df.columns and 'Close' in df.columns and 'Open' in df.columns:
        ...
        if 'ADOSC' in df.columns:
            # 优化：ADOSC由负转正是买方占优的重要信号
            adosc_turn_positive = (df['ADOSC'] > 0) & (df['ADOSC'].shift(1) <= 0)
            vol_price_score += np.where(
                adosc_turn_positive,  # 震荡指标由负转正
                3,
                np.where(
                    df['ADOSC'] > 0,  # 震荡指标为正
                    np.where(df['ADOSC'] > df['ADOSC'].shift(1), 2, 1),  # 震荡指标上升更好
                    0  # 震荡指标为负
                )
            )
        ...
        if 'MarketTrend' in df.columns:
            market_trend = df['MarketTrend']
            market_trend_score = np.where(
                market_trend > 0.01, 5,  # 强势上涨
                np.where(
                    market_trend > 0, 3,  # 温和上涨
                    np.where(market_trend < -0.01, 0, 1)  # 下跌趋势不加分，但盘整可以有1分
                )
            )
        ...
        if 'MarketSentiment' in df.columns:
            market_sentiment = df['MarketSentiment']
            market_sentiment_score = np.where(
                (market_sentiment > 0.5) & (market_sentiment < 2), 5,  # 适度活跃，最佳状态
                np.where(
                    (market_sentiment > 0.3) & (market_sentiment <= 0.5), 3,  # 低活跃度但有上升迹象
                    np.where(market_sentiment > 2, 2, 0)  # 过热得分低，过冷不加分
                )
            )
        
        # 综合市场因素得分 - 确保不超过10分
        df['市场因素得分'] = np.minimum(market_trend_score + market_sentiment_score, 10)
        
        # =====================================================================
        # 6. 反转信号得分 (最高5分) - 评估潜在反转机会
        # =====================================================================
        
        # 6.1 超跌反弹信号 (0-3分) - RSI超卖反弹
        ...
        if '均线突破' in df.columns:
            breakout_score = np.where(df['均线突破'], 2, 0)
        else:
            # 如果没有均线突破指标，使用MA10作为关键均线判断
            if all(col in df.columns for col in ['Close', 'MA10']):
                ma10_breakout = (df['Close'] > df['MA10']) & (df['Close'].shift(1) <= df['MA10'].shift(1))
                breakout_score = np.where(ma10_breakout, 2, 0)
        
        # 综合反转信号得分 - 确保不超过5分
        df['反转信号得分'] = np.minimum(oversold_score + breakout_score, 5)
        ...
        if '涨停概率' in df.columns:
            # 优化：涨停概率阈值降低到15%，更灵敏地捕捉潜在涨停股
            prob_values = df['涨停概率'].fillna(0).values
            limit_up_score = np.where(
                prob_values > 15,
                prob_values / 8,  # 15%得1.875分，40%得5分，80%得10分
                0
            )
            ...
            limit_up_score = np.maximum(0, np.minimum(limit_up_score, 10))  # 最高10分
        ...
        volume_score = df.get('量能得分', pd.Series([0.0] * len(df)))
        tech_score = df.get('技术指标得分', pd.Series([0.0] * len(df)))
        signal_score = df.get('短线信号得分', pd.Series([0.0] * len(df)))
        market_score = df.get('市场因素得分', pd.Series([0.0] * len(df)))
        reversal_score = df.get('反转信号得分', pd.Series([0.0] * len(df)))
        limit_up_score = df.get('涨停概率得分', pd.Series([0.0] * len(df)))
        consecutive_up_score = df.get('连续上涨得分', pd.Series([0.0] * len(df)))
        ...
        df['股票得分'] = 15.0 + \
                      ...
                      reversal_score * 1.5 + \
                      limit_up_score * 2.0 + \
                      consecutive_up_score * 0.8
        
        # 确保得分在0-100之间，并处理可能的NaN值
        df['股票得分'] = df['股票得分'].fillna(20.0).clip(0, 100)
        
        #计算次日上涨概率评级（严格）
        df['上涨潜力'] = '低'
        df.loc[df['股票得分'] > 70, '上涨潜力'] = '中'
        df.loc[df['股票得分'] > 80, '上涨潜力'] = '高'
        df.loc[df['股票得分'] > 90, '上涨潜力'] = '极高'
...
def generate_report(target_stocks: List[List[str]], tech: TechnicalIndicators, params: TradingParams):
    ...
    # 9. 按得分排序，取前50只股票
    report_df = report_df.sort_values(by="得分", ascending=False).head(50)
    ...
    report_file = os.path.join(Constants.BASE_DIR,"stock_r_report", f"stock_r_top50_{current_date}.csv")
    ...
    # 15. 创建进阶报告（包含更多指标和分析）
    advanced_report_file = os.path.join(Constants.BASE_DIR,"stock_r_report", f"stock_r_advanced_{current_date}.csv")
    report_df.to_csv(advanced_report_file, index=False, encoding='utf-8-sig')
    print(f"\n进阶分析报告已生成：{advanced_report_file}")
